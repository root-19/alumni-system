<x-layouts.app :title="isset($resume) ? __('Update Resume') : __('Create Resume')">
    <div class="max-w-4xl mx-auto mt-12 mb-12">
        <div class="bg-white shadow-lg rounded-2xl p-8">
            <h2 class="text-2xl font-bold text-gray-800 mb-6 border-b pb-3">
                {{ isset($resume) ? __('Update Your Resume') : __('Create Your Resume') }}
            </h2>
            ...
            <form action="{{ route('resumes.generate') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="full_name" value="{{ old('full_name', $resume->full_name ?? auth()->user()->name ?? '') }}" required
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $resume->email ?? auth()->user()->email ?? '') }}" required
                           class="w-full text-black px-4 py-2 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    @error('email')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="objective" rows="4" required
                              ...
                              placeholder="Write a brief statement about your career objectives and goals...">{{ old('objective', $resume->objective ?? '') }}</textarea>
                    @error('objective')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="educational_attainment" rows="6" required
                              class="w-full px-4 text-black py-2 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
                              placeholder="List your educational background (e.g., Bachelor's Degree in Computer Science, University Name, Year Graduated)">{{ old('educational_attainment', $resume->educational_attainment ?? '') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="training_seminars" rows="6"
                              class="w-full px-4 text-black py-2 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
                              placeholder="List any training programs, seminars, or workshops you've attended (e.g., Web Development Bootcamp, 2023)">{{ old('training_seminars', $resume->training_seminars ?? '') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="work_experience" rows="8"
                              class="w-full px-4 text-black py-2 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
                              placeholder="List your work experience (e.g., Software Developer at ABC Company, 2020-2023 - Responsibilities and achievements)">{{ old('work_experience', $resume->work_experience ?? '') }}</textarea>
                ...
                <div class="flex justify-end pt-4 border-t">
                    <button type="submit" 
                        ...
                        {{ isset($resume) ? 'Update Resume' : 'Generate Resume' }}
                    </button>
                </div>
            </form>
        </div>
